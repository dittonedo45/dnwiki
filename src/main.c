 ...
 tt:
   {
	...
	if( !myDD_AND_SAVER
	    ( wikipath ( sp, type ? type : "pedia", lang ? lang : "en" ), &r,
	      &j ) ) {
	 fprintf ( stderr, "OK (%s)\n", exxt );
	} else {
	 free ( sp );
...
#define wriset(x) (opt&(1<<x))
 ...
 if( !wrset ( 1 ) && !stitle ) {
  fprintf ( stderr, "Consider specifying the, --title, -T <arg> option.\n" );
  return 2;
 }
...
/* {{{ */
struct Mopts MainMS[] = {
 {WK_DOWNLOAD, "download|get|dw|ge", wk_download, "The Wiki Downloader."},
 {WK_VIEW, "reader|viewer|opener|surfer", wd_reader, "The Wiki Reader."},
 {.str = NULL,.main = NULL}
};
